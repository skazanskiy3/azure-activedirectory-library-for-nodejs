...
var OAuth2ClientAssertionType = constants.OAuth2.ClientAssertionType;
var OAuth2GrantType = constants.OAuth2.GrantType;
var OAuth2Scope = constants.OAuth2.Scope;
var OAuth2RequestedTokenUse = constants.OAuth2.RequestedTokenUse;
...
* @param  {AcquireTokenCallback} callback
 */
TokenRequest.prototype.getTokenOnBehalf = function(clientSecret, userAccessToken, username, callback) {
    this._log.info('Getting on-behalf-of token.')
    this._userId = username;

    this._getTokenWithCacheWrapper(callback, function(getTokenCompleteCallback) {
      var oauthParameters = this._createOAuthParameters(OAuth2GrantType.JWT_BEARER);

      oauthParameters[OAuth2Parameters.CLIENT_SECRET] = clientSecret;
      oauthParameters[OAuth2Parameters.ASSERTION] = userAccessToken;
      oauthParameters[OAuth2Parameters.REQUESTED_TOKEN_USE] = OAuth2RequestedTokenUse.ON_BEHALF_OF;
...
TokenRequest.prototype.getTokenWithCertificate = function(certificate, thumbprint, callback) {
  ...
  var oauthParameters = this._createOAuthParameters(OAuth2GrantType.CLIENT_CREDENTIALS);
  oauthParameters[OAuth2Parameters.CLIENT_ASSERTION_TYPE] = OAuth2ClientAssertionType.JWT_BEARER;
